...
struct LinearProber : public Prober<KeyType> {
   ...
   HASH_INDEX_T next()
   {
       ...
       HASH_INDEX_T curIdx = (this->numProbes_ + this->start_) % this->m_;
...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
   double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
      :  hash_(hash), kequal_(kequal), prober_(prober), mIndex_(0), resizeAlpha(resizeAlpha)
{
...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
 for(int i=0; i<table_.size();i++) delete table_[i];
}
...
{
    double rehash = lf / (double) CAPACITIES[mIndex_];
    if(rehash >= this->resizeAlpha) resize();


    HASH_INDEX_T h = this->probe(p.first);

    if((npos == h)) throw std::logic_error("There is no space left to insert");
    else if(table_[h] != NULL){
...
void HashTable<K,V,Prober,Hash,KEqual>::remove(const KeyType& key)
{
    ...
    if(npos == h || table_[h] == NULL) return; 

    if(table_[h]->deleted == false){
        sizeC--;
...
{
   if((npos == this->probe(key)) || table_[this->probe(key)] == NULL) return NULL;
   return table_[this->probe(key)]->item;
}
...
{
   if((npos == this->probe(key)) || nullptr == table_[this->probe(key)] ) return nullptr;
   return &table_[this->probe(key)]->item;
}
...
{
   HashItem const* curItem = this->internalFind(key);

   if(curItem == NULL) throw std::out_of_range("Invalid key");
   return curItem->curItem.second;
}
...
{
   HashItem* curItem = this->internalFind(key);
...
typename HashTable<K,V,Prober,Hash,KEqual>::HashItem* HashTable<K,V,Prober,Hash,KEqual>::internalFind(const KeyType& key) const
{
   ...
   if((NULL == table_[this->probe(key)]) || (npos == this->probe(key)) ) return nullptr;
   return table_[this->probe(key)];
}
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    mIndex_++;
    std::vector<HashItem*> curHash = std::move(table_);
    ...
    for(auto& itemTo : curHash){

        if(itemTo != NULL && itemTo->deleted == false) this->insert(itemTo->item);
        delete itemTo;
...
void HashTable<K, V, Prober, Hash, KEqual>::reportAll(std::ostream& out) const
{
    ...
    {
        if(table_[i] != nullptr) out << "Bucket " << i << ": " << table_[i]->item.first << " " << table_[i]->item.second << std::endl;
    }
}
